...
s_scream_show g_scream;



void Scream_DEV_Init()
{
...
void Scream_init()
{
	...
	g_scream.Imagesize = ((EPD_2IN9_V2_WIDTH % 8 == 0)? (EPD_2IN9_V2_WIDTH / 8 ): (EPD_2IN9_V2_WIDTH / 8 + 1)) * EPD_2IN9_V2_HEIGHT;
	g_scream.max_width = EPD_2IN9_V2_WIDTH;
	g_scream.max_hight = EPD_2IN9_V2_HEIGHT;

	if((g_scream.BlackImage = (UBYTE *)malloc(g_scream.Imagesize)) == NULL) {
		Debug("Failed to apply for black memory...\r\n");
		//return -1;
	}
	memset(g_scream.BlackImage , 0 , g_scream.Imagesize);
	EPD_2IN9_V2_Init_Fast();
	Paint_NewImage(g_scream.BlackImage, EPD_2IN9_V2_WIDTH, EPD_2IN9_V2_HEIGHT, 90, WHITE);	
...
void Scream_show_page_test()
{

	// 1.Select Image
	Paint_SelectImage(g_scream.BlackImage);
...
void Scream_print_page(s_page *page,unsigned int page_id)
{
...
void Screen_print_page(s_screen_show *screen,char *acode)
{
	s_page *p_frist_page = screen->page_linklist;
	static s_page *p_page = screen->page_linklist;
	char fread_buf[40] = {0};
	...
	sprintf(fread_buf,"page %d//%d",p_page->current_page_id,scream->page_num);
...
//初始化s_scream_show
void s_scream_show_init(s_scream_show *scream)
{
	memset(scream , 0 ,sizeof(scream));
}


//选择文件
void scream_select_file()
{

}
#if 0
//显示txt文字
void scream_show_page(s_page *p_page,unsigned int page_id)
{
	unsigned int i,j,k;
	unsigned int page_seek = 0;
	char fread_buf[40] = {0};
